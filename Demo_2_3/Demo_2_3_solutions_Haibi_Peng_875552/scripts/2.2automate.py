...
if __name__ == '__main__':
    args = sys.argv
    spine_switch_num = int(input("Enter the spine number: "))
    leaf_switch_num = int(input("Enter the leaf number: "))
    host_node = int(input("Enter the host node number for each leaf switch: "))
    ...
    for i in range(1, leaf_switch_num + 1):
        for j in range(2, host_node + 2):
            create_lxc_container("lxc-{}-{}".format(i, j - 1), "ovs-{}".format(i + spine_switch_num), 2, "10.0.{}.{}".format(i, j), i)